...
struct Levels: View {
    ...
    @State private var currentUserLevel : Int = 4
...
//                                        .frame(width: 60, height: 70)
                                ...
                                HStack(spacing: 50){
                                    Button(action:{
                                        if(currentUserLevel >= currentEnemy.level){
                                    ...
                                    .disabled(currentUserLevel < currentEnemy.level ? true : false)
                            ...
                            .overlay((currentUserLevel < currentEnemy.level) ? (RoundedRectangle(cornerRadius: 25).allowsHitTesting(false).frame(width: (UIScreen.screenWidth)/3)) .foregroundColor(Color.white.opacity(0.7)) : (RoundedRectangle(cornerRadius: 25).allowsHitTesting(false).frame(width: (UIScreen.screenWidth)/3)) .foregroundColor(Color.white.opacity(0.0)))
                            .overlay(
                                Image(systemName: checkLevel(currentEnemy: currentEnemy) ? "" : "lock")
                                    .resizable()
                                    .frame(width: 60, height: 70))
...
//        }// NavigationView ends
    ...
    func checkLevel(currentEnemy : Enemy) -> Bool{
        if (self.currentUserLevel >= currentEnemy.level){
            return true
        }
        return false
    }
