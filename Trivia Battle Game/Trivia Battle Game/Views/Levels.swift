...
struct Levels: View {
    ...
    private var interstitialHelper : InterstitialHelper
    
    init() {
        self.interstitialHelper = InterstitialHelper()
    }
...
//                                                    interstitialHelper.showAd()
                                                    if(self.fireDBHelper.user.db_Energy < 1){
                                                        self.errorMessage = "Sorry you don't have enough energy"
                                            ...
                                            .alert(isPresented: self.$errorFound){
...
//                Text("Energy \(self.fireDBHelper.user.db_Energy)/10")
...
//                    .padding(0)
                ...
                VStack(spacing:0){
                    ...
                    ZStack(alignment: .leading){
                        ...
                        RoundedRectangle(cornerRadius: 20, style: .continuous)
                            .frame(width:
                                    CGFloat(self.fireDBHelper.user.db_Energy*75)/CGFloat(10), height: 12)
                            .foregroundColor(self.fireDBHelper.user.db_Energy <= 3 ? .red : (self.fireDBHelper.user.db_Energy > 3 && self.fireDBHelper.user.db_Energy <= 6) ? .yellow : .green)
                    }
                    .padding(.top, 5)
...
//        }// NavigationView ends
        .onAppear(perform: {
//            print("--2 \(self.fireDBHelper.user.db_dateRecharge.formatted())")
