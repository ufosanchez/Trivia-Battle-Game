...
class FireDBHelper : ObservableObject{
    ...
    private let FIELD_ENERGY : String = "db_Energy"
    private let FIELD_RECHARGE : String = "db_Recharge"
    private let FIELD_DATE_RECHARGE : String = "db_dateRecharge"
...
//    }
                ...
                try self.store
                    ...
                    .setData ([FIELD_USERNAME : newUser.db_Username,
                         ...
                         FIELD_CORR_QUESTIONS : newUser.db_CorrectQuestions,
                                 ...
                                 FIELD_ENERGY : 10,
                               FIELD_RECHARGE : newUser.db_Recharge,
                          FIELD_DATE_RECHARGE : newUser.db_dateRecharge,
                             FIELD_CHAMPINDEX : newUser.pChampIndex,
        ...
        if (loggedInUserEmail.isEmpty){
            ...
            self.store
                ...
                .addSnapshotListener { documentSnapshot, error in
                    ...
                    self.user.db_dateAdded = (data ["db_dateAdded"] as? Timestamp)?.dateValue() ?? Date()
                    self.user.db_Energy = data ["db_Energy"] as? Int ?? 0
                    self.user.db_Recharge = data ["db_Recharge"] as? Bool ?? false
                    self.user.db_dateRecharge = (data ["db_dateRecharge"] as? Timestamp)?.dateValue() ?? Date()
                    print("dataa dateeee : \(self.user.db_dateAdded)")
                }
        }
    }
    
    func updateDataProfileGame(newLevel : Int, newQuestions : Int, newGames : Int, newCorrectQuestions : Int, newEnergy : Int){
        self.loggedInUserEmail = UserDefaults.standard.string(forKey: "KEY_EMAIL") ?? ""
        print("getUser - The user is : \(self.loggedInUserEmail)")
                ...
                if let document = document, document.exists {
                    docRef
                        .updateData([FIELD_LEVEL : newLevel,
                            ...
                            FIELD_CORR_QUESTIONS : newCorrectQuestions,
                                    FIELD_ENERGY : newEnergy]) { error in
                            if let error = error {
                                print(#function, "Unable to update document : \(error)")
                            } else {
                                print(#function, "Successfully update document")
    ...
    func updateRechargeBool (rechargeBool: Bool) {
        self.loggedInUserEmail = UserDefaults.standard.string(forKey: "KEY_EMAIL") ?? ""
        ...
        docRef.getDocument { [self] (document, error) in
            if let document = document, document.exists {
                docRef
                    .updateData([FIELD_RECHARGE : rechargeBool,
                            FIELD_DATE_RECHARGE : Date()]) { error in
                        if let error = error {
                            print(#function, "Unable to update document : \(error)")
                        } else {
                            print(#function, "Successfully update document")
                ...
                do {
                    try self.store
                        .collection(COLLECTION_USER)
                        .document(loggedInUserEmail)
                        .setData ([FIELD_RECHARGE : rechargeBool,
                              FIELD_DATE_RECHARGE : Date()])
                } catch let error as NSError {
                    print(#function, "Unable to add document to firestore : \(error)")
                }
            }
    ...
    func updateEnergy (rechargeBool: Bool, eneryRecharge : Int, newDate : Date) {
        self.loggedInUserEmail = UserDefaults.standard.string(forKey: "KEY_EMAIL") ?? ""
        ...
        docRef.getDocument { [self] (document, error) in
            if let document = document, document.exists {
                docRef
                    .updateData([FIELD_RECHARGE : rechargeBool,
                                   FIELD_ENERGY : eneryRecharge,
                            FIELD_DATE_RECHARGE : newDate]) { error in
                        if let error = error {
                            print(#function, "Unable to update document : \(error)")
                        } else {
                            print(#function, "Successfully update document")
    ...
    func updateUsername (newUserName: String) {
        ...
        docRef.getDocument { [self] (document, error) in
            if let document = document, document.exists {
                docRef
                    .updateData([FIELD_USERNAME : newUserName]) { error in
                        if let error = error {
                            print(#function, "Unable to update document : \(error)")
                        } else {
                            print(#function, "Successfully update document")
                ...
                do {
                    try self.store
                        .collection(COLLECTION_USER)
                        .document(loggedInUserEmail)
                        .setData ([FIELD_USERNAME : newUserName])
                } catch let error as NSError {
                    print(#function, "Unable to add document to firestore : \(error)")
                }
            }
